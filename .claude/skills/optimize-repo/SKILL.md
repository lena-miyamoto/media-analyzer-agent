...
This project uses `.claude/` as the sole source of truth for all skills and agents. `.github/` contains thin wrappers
for Copilot.

| Surface | Owner / Shape |
|---------|---------------|
| Repo workflow, source priority, tool preferences, shared rules | `CLAUDE.md`; main instruction file — all shared rules live here. |
| Claude entrypoint | `CLAUDE.md` (this file). |
| Copilot entrypoint | `.github/copilot-instructions.md`; thin pointer to `CLAUDE.md` and Copilot wrappers. |
| Shared skills | `.claude/skills/<name>/SKILL.md`; owns actual procedure. Copilot routes through `.github/skills/` wrappers. |
| Copilot skill wrappers | `.github/skills/<name>/SKILL.md`; frontmatter plus one pointer line to `.claude/`. |
...
| Agent/skill resource dirs | `.claude/skills/<name>/rules/`; on-demand reference files for oversized instruction files (see 500-line rule). |
...
| Domain | Owner |
|--------|-------|
| Shared repo workflow, source priority, tool preferences, output rules | `CLAUDE.md` |
| Article summarization procedure | `.claude/skills/summarize-article/SKILL.md` |
| Book/essay summarization procedure | `.claude/skills/summarize-book/SKILL.md` |
| Video summarization procedure | `.claude/skills/summarize-video/SKILL.md` |
| Repo optimization and audit procedure | `.claude/skills/optimize-repo/SKILL.md` |
| Instruction file compression | `.claude/skills/compress-skill/SKILL.md` |
| Media archive access | `.claude/skills/media-db/SKILL.md` |
...
3. Capture the current state before editing:
   ...
   - Count relevant instruction lines with `wc -l CLAUDE.md AGENTS.md README.md .claude/skills/*/SKILL.md
     .github/skills/*/SKILL.md .claude/agents/*.md .github/agents/*`.
...
4. Audit against the structure above. **Before evaluating any file, read
   ...
   - `CLAUDE.md` should ideally stay under 80 lines but must never exceed 150 lines and keep source priority as a table.
...
5. Repair messy instructions in this order:
   ...
   - Replace copied rules with short pointers such as `Follow .claude/skills/<name>/SKILL.md.` or `Follow CLAUDE.md.`
...
7. Re-check after edits:
   - Names, descriptions, and argument hints aligned across `.github` and `.claude` surfaces.
...
→ `.claude/skills/summarize-video/SKILL.md` only.

`html2text`, `-utf8`, `-width 100`, `-nobs`
→ `CLAUDE.md` only (tool preference).
...
→ `CLAUDE.md` only (source priority).

`de.wikipedia.org`, `en.wikipedia.org`
→ `CLAUDE.md` only (Wikipedia preference).

`www.sci-hub.st`, `Sci-Hub`
→ `CLAUDE.md` only (full-text fallback).

`DocCheck`, `Flexicon`, `flexikon.doccheck.com`
→ `CLAUDE.md` only (medical wiki).

`## Summary`, `## Most Important Points`
→ `.claude/skills/summarize-article/SKILL.md` and `.claude/skills/summarize-video/SKILL.md` only.

Author biography / other works / historical context prohibitions
→ `CLAUDE.md` only.

`tmp/`
→ `CLAUDE.md` only (scratch output dir).

No local archive, no local database
→ `CLAUDE.md` only.

`.pymarkdown.yaml`
→ Config file. `CLAUDE.md` may reference the Markdown standard.
...
1. **Confirm `.claude/skills/optimize-repo/rules/context-engineering-best-practices.md` was read** before any
...
8. Confirm `CLAUDE.md` is compact and source priority rules remain clear.
